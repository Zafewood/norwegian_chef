...
def run_episode(env, a0_policy, a1_policy, visual=False):
    ...
    while True:
        ...
        with torch.no_grad():
            a0_obs = torch.tensor(env.featurize_state_mdp(state)[0], dtype=torch.float32) #get the first player's observation as a tensor
            a1_obs = torch.tensor(env.featurize_state_mdp(state)[1], dtype=torch.float32) #get the second player's observation as a tensor
            a0_action_probs = torch.softmax(a0_policy(a0_obs), dim=-1)
            a0_action = torch.multinomial(a0_action_probs, 1)

            a1_action_probs = torch.softmax(a1_policy(a1_obs), dim=-1)
            a1_action = torch.multinomial(a1_action_probs, 1)

        joint_action = (Action.ALL_ACTIONS[a0_action.item()], Action.ALL_ACTIONS[a1_action.item()])
        state, timestep_sparse_reward, done, info = env.step(joint_action)
...
if __name__ == "__main__":
    ...
    #Load policies
    a0_policy = PolicyNetwork(obs_dim, act_dim)
    a0_policy.load_state_dict(torch.load(f"trained/{test_model}_a0.pth"))
    a0_policy.eval()

    a1_policy = PolicyNetwork(obs_dim, act_dim)
    a1_policy.load_state_dict(torch.load(f"trained/{test_model}_a1.pth"))
    a1_policy.eval()

    for episode in range(1,num_testing_episodes+1):
        total_raw_reward, soup_reward = run_episode(env, a0_policy, a1_policy, visual)
